...
def evaluate():
    ...
    policy.load_state_dict(torch.load("stage7_52_global_fire_channel_200updates.pth", map_location=device))
    policy.eval()
    ...
    for ep in range(20):
        ...
        while not done:
            ...
            if len(active_coords) > 0:
                ...
                if prev_dist is not None:
                    if current_fire_dist < prev_dist: steps_closer += 1
                    elif current_fire_dist > prev_dist: steps_farther += 1
                    else: steps_same += 1
                prev_dist = current_fire_dist
                if init_dist is None: init_dist = current_fire_dist
        ...
        if not hasattr(env, 'total_closer'): env.total_closer = 0
        if not hasattr(env, 'total_farther'): env.total_farther = 0
        if not hasattr(env, 'total_same'): env.total_same = 0
        if not hasattr(env, 'init_dists'): env.init_dists = []
        if not hasattr(env, 'final_dists'): env.final_dists = []
        env.total_closer += steps_closer
        env.total_farther += steps_farther
        env.total_same += steps_same
        if init_dist is not None: env.init_dists.append(init_dist)
        if prev_dist is not None: env.final_dists.append(prev_dist)
    ...
    tot = env.total_closer + env.total_farther + env.total_same
    print(f"Steps moving closer:      {env.total_closer/tot*100:.1f}%")
    print(f"Steps moving farther:     {env.total_farther/tot*100:.1f}%")
    print(f"Steps same distance:      {env.total_same/tot*100:.1f}%")
    print(f"Initial fire distance:    {np.mean(env.init_dists):.2f}")
    print(f"Final fire distance:      {np.mean(env.final_dists):.2f}")
