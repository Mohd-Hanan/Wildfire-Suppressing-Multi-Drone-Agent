...
class PPOTrainer:
    def __init__(self, policy, device):
        ...
        actor_params = (list(policy.actor_cnn.parameters()) + 
                        list(policy.actor_vec.parameters()) + 
                        list(policy.actor_fusion.parameters()) + 
                        list(policy.actor_head.parameters()))
        critic_params = (list(policy.critic_cnn.parameters()) + 
                         list(policy.critic_vec.parameters()) + 
                         list(policy.critic_fusion.parameters()) + 
    ...
    def update(self, spatial, drone, wind, acts, old_log_probs, advantages, returns):
        ...
        _, new_log_probs, entropy, new_values = self.policy.get_action_and_value(obs, acts)
