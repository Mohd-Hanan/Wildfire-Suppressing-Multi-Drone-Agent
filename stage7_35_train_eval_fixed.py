...
# The real SmokeTestTrainer expects these properties to create its optimizers
# so we map our new architecture modules to the old names temporarily just for the trainer
class PatchedTrainer(SmokeTestTrainer):
    def __init__(self, policy, device):
        # Temporarily patch policy so super().__init__ grabs all parameters
        policy.actor_mlp = nn.ModuleList([policy.actor_vec, policy.actor_fusion])
        policy.critic_mlp = nn.ModuleList([policy.critic_vec, policy.critic_fusion])
        super().__init__(policy, device)
        # Restore so we don't pollute anything
        del policy.actor_mlp
        del policy.critic_mlp

def train():
    print("========================================")
    print("STAGE 7.35 — 20-UPDATE SYMMETRIC TRAINING (FIXED PPO LOOP)")
    print("========================================")
    ...
    trainer = PatchedTrainer(policy=policy, device=device)
