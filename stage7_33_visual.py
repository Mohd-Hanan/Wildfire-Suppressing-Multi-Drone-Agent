...
def main():
    print("========================================")
    print("STAGE 7.33 — VISUAL EVALUATION (200 UPDATES)")
    print("========================================")
    ...
    print(f"\n--- VISUAL EPISODE {ep} ---")
    print(f"{'Step':>5} | {'Action':<15} | {'Position':<10} | {'Bat':>4} | {'Pay':>3} | {'Nearest Fire (dx,dy)'}")
    print("-" * 75)
    ...
    while not (terminated or truncated):
        ...
        # Fire info from observation vector: (dx, dy)
        fire_dx = obs["drone"][6] * eval_env.world.width
        fire_dy = obs["drone"][7] * eval_env.world.height
        
        # Print diagnostic every 10 steps (and step 0)
        if step % 10 == 0:
            act_str = f"{action} ({action_names[action]})"
            print(f"{step:5d} | {act_str:<15} | {pos_str:<10} | {drone.battery:4d} | {drone.payload:3d} | ({fire_dx:5.1f}, {fire_dy:5.1f})")
            
        obs, reward, terminated, truncated, info = eval_env.step(action)
        total_reward += reward
        step += 1
        clock.tick(15) # Controls the Pygame visual speed
